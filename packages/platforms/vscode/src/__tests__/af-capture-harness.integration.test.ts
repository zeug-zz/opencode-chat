 ...
 * launches a shell, a subprocess, or a live AF command.
 */
...
describe("Vibefeld AF live-capture harness integration", () => {
  ...
  captureDescribe("explicit live capture", () => {
    it("captures the fixed CLI shapes, sanitizes them, and removes the disposable root", async () => {
      ...
      try {
        ...
        const outputs = { version: "", schema: "", init: "", status: "" };

        for (const operation of ["version", "schema", "init", "status"] as const) {
          ...
          if (operation === "init") expect(argv[4]).toBe(AF_CAPTURE_AUTHOR);
