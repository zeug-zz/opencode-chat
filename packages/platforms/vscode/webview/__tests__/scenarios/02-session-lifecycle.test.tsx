...
describe("セッションライフサイクル", () => {
  ...
  it("セッション切り替え後に旧レビュー結果を表示しないこと", async () => {
    renderApp();
    const sessionA = createSession({ id: "review-session-a" });
    const sessionB = createSession({ id: "review-session-b" });
    ...
    await sendExtMessage({
      type: "messages",
      sessionId: sessionA.id,
      messages: [{ info: message, parts: [createTextPart("Reviewed response", { messageID: message.id })] }],
    });
    await sendExtMessage({
      type: "reasoningReview",
      sessionId: sessionA.id,
      summary: {
        reviewedMessageId: message.id,
        status: "conditional",
        invocation: "manual",
        conclusion: "Session A review",
        assumptions: [],
        evidenceStatus: "unverified",
        openChallenges: [],
      },
    });
    expect(screen.getByText("Session A review")).toBeInTheDocument();

    await sendExtMessage({ type: "activeSession", session: sessionB });
    expect(screen.queryByText("Session A review")).not.toBeInTheDocument();
    await sendExtMessage({
      type: "reasoningReview",
      sessionId: sessionA.id,
      summary: {
  ...
  it("clears review cards when the reviewed session is deleted", async () => {
    renderApp();
    const session = createSession({ id: "deleted-review-session" });
    const message = createMessage({
      id: "deleted-review-message",
    ...
    await sendExtMessage({
      type: "messages",
      sessionId: session.id,
      messages: [{ info: message, parts: [createTextPart("Response", { messageID: message.id })] }],
    });
    await sendExtMessage({
      type: "reasoningReview",
      sessionId: session.id,
    ...
    await sendExtMessage({ type: "event", event: { type: "session.deleted", properties: { info: session } } as any });
