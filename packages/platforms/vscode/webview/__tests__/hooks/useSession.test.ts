...
describe("useSession", () => {
  ...
  context("セッション一覧を管理する場合", () => {
    ...
    it("setSessions でセッション一覧を設定できること", () => {
      const { result } = renderHook(() => useSession(createSessionRef()));
  ...
  context("session.status イベントを受信した場合", () => {
    ...
    it("アクティブセッションが busy のとき sessionBusy が true になること", () => {
      const ref = createSessionRef({ id: "active" } as ChatSession);
      const { result } = renderHook(() => useSession(ref));
      const event = {
        type: "session.status",
    ...
    it("アクティブセッションが idle のとき sessionBusy が false になること", () => {
      const ref = createSessionRef({ id: "active" } as ChatSession);
      const { result } = renderHook(() => useSession(ref));
      const busyEvent = {
        type: "session.status",
      ...
      const idleEvent = {
        type: "session.status",
        properties: { sessionID: "active", status: { type: "idle" } },
      } as unknown as AgentEvent;
      act(() => result.current.handleSessionEvent(idleEvent));
      expect(result.current.sessionBusy).toBe(false);
    });

    // foreign session status does not affect busy
    it("別セッションの busy は sessionBusy に影響しないこと", () => {
      const ref = createSessionRef({ id: "active" } as ChatSession);
      const { result } = renderHook(() => useSession(ref));
      const foreignBusy = {
        type: "session.status",
      ...
      act(() => result.current.handleSessionEvent(foreignBusy));
      expect(result.current.sessionBusy).toBe(false);
    });

    // foreign session idle does not clear busy
    it("別セッションの idle は sessionBusy をクリアしないこと", () => {
      const ref = createSessionRef({ id: "active" } as ChatSession);
      const { result } = renderHook(() => useSession(ref));
      act(() =>
      ...
      act(() => result.current.handleSessionEvent(foreignIdle));
      expect(result.current.sessionBusy).toBe(true);
    });
  });
