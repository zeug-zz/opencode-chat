...
- Reasoning Review Enable and workspace Opt out preferences now persist when a compatible runtime is available.
...
- Restored SHA-pinned required CodeQL analysis for Dependabot and `main`.
- Patched the transitive `lodash-es` audit path.
...
- Fixed the context-memory chip to report the latest valid prompt-context snapshot rather than cumulative prompt work across messages and steps.
