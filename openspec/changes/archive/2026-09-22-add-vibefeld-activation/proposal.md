...
- Compose activation in `extension.ts`: construct the bridge and proof store beneath `context.globalStorageUri`, preflight once per activation, and inject `ClaimProjectionReasoningReviewController` only when the bridge is ready and reports an explicitly supported claim operation; otherwise inject `UnavailableReasoningReviewController` and publish the bounded `claim-capability-unavailable` runtime status over the existing protocol.
- Add a Global-target preference with a per-workspace opt-out and an availability-gated `ToolConfigPanel` control; gate the per-message review affordance on the same review availability, hide it entirely when review is unavailable, and toggle a completed review's card open and closed (localized show/hide labels, no re-run on toggle); synchronized locale keys land in every dictionary, and no non-functional control is rendered when AF is absent or incompatible.
...
- `vibefeld-claim-projection`: the projection dormancy requirement is amended so the claim-projection controller may be injected dynamically only when the runtime is ready and the bridge reports an explicitly supported claim operation, with ordinary activation behavior unchanged when dormant or when the claim capability is absent.
