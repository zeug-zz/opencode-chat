...
The scaffold SHALL NOT discover or launch an external runtime, create or write a
proof workspace, create or modify configuration, add an OpenCode plugin source,
MCP server, custom tool, or agent overlay, or change the permissions or sandbox
policy of Chat, Scout, Write, the research worker, or the independent TUI.

#### Scenario: Scaffold handles a review request

- **WHEN** the unavailable controller handles a manual review request
