...
- Disable repository-level CodeQL default setup before re-running the versioned workflow's PR checks, because GitHub rejects advanced CodeQL SARIF uploads while default setup is enabled; then verify advanced setup is the only active CodeQL path before merge.
...
A misnamed job would remain blocked by the existing ruleset, so the exact required names are a release gate. GitHub rejects advanced CodeQL SARIF uploads while default setup is enabled, so a brief pre-merge default-setup disable is mandatory; if the re-run fails, restore default setup through the repository CodeQL configuration rather than weakening required checks. Running pull-request code with `pull_request_target` would expose elevated repository credentials and is prohibited. No extension, sandbox, nono, or user-facing behavior changes.