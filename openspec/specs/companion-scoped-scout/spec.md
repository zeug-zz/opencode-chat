...
The companion-scoped Scout configuration SHALL deny editing and shell execution. Scout SHALL be allowed to use task delegation only for the exact extension-injected agent named `chat-research-worker`; all other task targets SHALL remain denied by a wildcard default. Scout SHALL retain read-only workspace discovery, documentation, web research, clarification, and only the exact capability-gated Hindsight recall/search, reflection, and confirmation-gated retention tools. The retention exception SHALL not depend on a workspace retention enablement setting, but SHALL require exact provider identity, capability, observed-tool, lifecycle, sandbox, payload, and confirmation checks. This exception SHALL not grant local file editing, shell execution, general writes, task recursion, package control, terminal control, provider administration, deletion, or unknown tools. Inherited native plugins SHALL NOT broaden Scout or Build-backed Write permissions, and SHALL NOT grant either agent arbitrary plugin tools. The GUI SHALL retain `build` as the explicit agent for editing and command execution. Build-backed Write SHALL receive only the same exact capability-gated Hindsight recall/search/reflection tools and, when the exact verified retention operation permits it, the same exact confirmation-gated retention operation; it SHALL retain its deny-by-default boundary and receive no other escalation.
...
- **AND** Hindsight administration, deletion, diagnostic, synchronization, and unknown tools SHALL remain denied except for the exact retention exception below
...
- **WHEN** the exact provider identity, retain capability, observed retention tool, lifecycle, sandbox, payload, or confirmation checks do not pass
- **THEN** Scout SHALL not receive any retention operation
- **AND** Build SHALL not receive any retention operation
- **WHEN** the exact provider identity, retain capability, observed retention tool, lifecycle, sandbox, and payload checks pass
...
- **AND** Write SHALL not receive Hindsight deletion, administration, diagnostic, synchronization, or unknown tools except for the exact retention exception above
