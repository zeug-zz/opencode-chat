...
- `nono` は任意の外部ツールです。Scribe はバイナリをバンドル・インストールせず、`nono`、そのプロファイル、または OpenCode/Hindsight の設定を構成・変更しません。
- 対応する macOS/Linux で Chat サンドボックスが有効な場合、Scribe は外部 nono の組み込み `opencode` プロファイルを既定で使用し、起動前に検証します。`$XDG_CONFIG_HOME/nono/profiles`（未設定時は `~/.config/nono/profiles`）にカスタムプロファイルが検出され、保存済みの選択がない場合だけ、組み込み `opencode` またはカスタムプロファイルを選ぶノンブロッキングなピッカーを一度表示します。カスタム選択は任意で、閉じるか既定値を選ぶと `opencode` のままです。Scribe が保存するのは明示的なプロファイル名だけで、プロファイル JSON を解析・変更せず、カスタムプロファイルを自動選択しません。既定・カスタムのいずれも有効なプロファイルなら native Hindsight を使います。

  ```sh
  nono profile show <name>
  ```

- nono が起動前に利用できない（実行ファイルがない、実行できない、または未対応）、あるいは実行ファイル／有効なプロファイルの事前検証に失敗した場合だけ、既存の互換サンドボックスへフォールバックし、Hindsight は検索・一覧・読み取りだけです。nono 選択後の起動・実行時失敗は別のバックエンドへダウングレードせず、フェイルクローズします。nono と互換サンドボックスは別個の強制ポリシーであり、オプトイン検証なしに同等とは扱いません。統合チェックは `OPENCODE_CHAT_RUN_NONO_INTEGRATION=1` と `OPENCODE_CHAT_NONO_PROFILE=opencode` の両方が必要です。明示的に選択したカスタムプロファイルを検証する場合は後者をその名前に置き換え、ネットワーク確認を行う場合だけ `OPENCODE_CHAT_RUN_NONO_NETWORK=1` を追加します。
